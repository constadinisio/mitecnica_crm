...
- **Fase 2C** ✅ — hardening, UX y observabilidad ligera: health/ready endpoints (`/health`, `/ready`, `/api/v1/health`), operational summary (`/api/v1/dashboard/operational-summary`), exportaciones CSV (institutions, payments, audit), navegación cruzada entre institución ↔ suscripción ↔ pago ↔ auditoría, chips + badges + diff en el módulo de auditoría, card "Pulso operativo" en el dashboard, empty states diferenciados (con filtro vs sin datos) y permisos `exports.*` espejados en backend + CRM.
- **Fase 3 (integración tenant)** ✅ — webhook emitter con outbox durable (`crm_webhook_outbox`), HMAC-SHA256, reintentos con backoff exponencial. Hooks en service layer de `institutions`, `subscriptions`, `institutionModules` que emiten `tenant.created`/`suspended`/`reactivated`/`archived`/`plan_changed`/`modules_changed`. Endpoint `/api/v1/integration/institutions/sync-feed` con auth por API key estática para reconciliación diaria del tenant. Mapeo de 6 estados CRM → 3 buckets tenant (activo/suspendido/archivado). Ver [INTEGRATION_SETUP.md](./docs/INTEGRATION_SETUP.md).

Quedan para próximas fases: provisioning técnico real, billing automation, pasarela de pagos real, UI completa de usuarios CRM, soporte/tickets, configuración avanzada.
